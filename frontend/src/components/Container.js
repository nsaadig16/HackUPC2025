    ...
    <div className="h-full">
      <div
        className={`transition-opacity duration-500 h-full ${
          isVisible2 ? 'opacity-100' : 'opacity-0 hidden'
        }`}
      >
        <div className="flex flex-row justify-center items-center h-full">
            ...
            <div className="flex flex-row w-full">
              <div className="w-28 h-12">
                <button
                  ...
                  className={`ml-5 h-full w-24 text-xl ${
                    spaceActive === 1
                      ? 'text-blue-700 border-b-2 border-blue-700'
                      : 'text-black border-none hover:text-gray-700 hover:border-b-2 hover:border-gray-700'
                  }`}
                >
                  Resultats
                </button>
              ...
              <div className="w-28 h-12">
                <button
                  ...
                  className={`h-full text-xl w-24 ${
                    spaceActive === 1
                      ? 'text-black border-none'
                      : 'text-blue-700 border-blue-700 border-b-2'
                  } hover:text-gray-700 hover:border-b-2 hover:border-gray-700`}
                >
                  Grup
                </button>
              </div>
                ...
                <>
                  ...
                  <div className="flex flex-col w-full basis-[40%] bg-white p-6">
                    ...
                    <div className="flex justify-center items-center space-x-6 w-full mt-auto">
                      <button
                        className="bg-green-50 hover:bg-green-100 text-green-600 font-bold py-3 px-8 rounded-full flex items-center transition-colors border border-green-100"
                        onClick={() => {
                          // Lógica para aceptar
                          console.log('Aceptar');
                        }}
                      ...
                      <button
                        className="bg-red-50 hover:bg-red-100 text-red-600 font-bold py-3 px-8 rounded-full flex items-center transition-colors border border-red-100"
                        onClick={() => {
          ...
          <div className="basis-3/12 w-full flex flex-col h-full border-2">
            <div className="flex flex-col w-full h-12 border-b-2 justify-center items-center">
              <h2>Shedule Master</h2>
            </div>

            <div className="flex flex-col border-l-2 border-gray-300 h-full w-full overflow-y-auto">
              {acceptedList.map((item, index) => (
                <div
                  key={index}
                  className="flex flex-row justify-between items-center border-b-2 border-gray-300 p-2 rounded-md"
                >
                  <p>
                    {item.day}-{item.hour}. {item.name}
                  </p>
                </div>
              ))}
            </div>
      ...
      <div
        className={`transition-opacity duration-500 ${
          formVisible ? 'opacity-100' : 'opacity-0 hidden'
        }`}
      >
        <Form Type={group} />
      </div>
